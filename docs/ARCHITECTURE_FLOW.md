...
> - **Azure AI Search Knowledge Base (FoundryIQ)** = Knowledge retrieval ONLY (extractive RAG search, no LLM reasoning)
...
flowchart TB
    ...
    subgraph External["🌐 External Services"]
        subgraph FoundryIQ["Azure AI Search (FoundryIQ)"]
            BLOB[(Azure Blob Storage<br/>Policy Docs)]
            KS[Knowledge Source<br/>auto-indexes → local index]
            KB[(Knowledge Base<br/>LLM: gpt-4o<br/>Extractive Retrieval)]
            BLOB --> KS --> KB
        end
...
flowchart TB
    ...
    subgraph External["🌐 External Services"]
        subgraph FoundryIQ["Azure AI Search (FoundryIQ)"]
            ...
            KS2[Knowledge Source<br/>+ local index]
            RAG[(Knowledge Base)]
            BLOB2 --> KS2 --> RAG
        end
        MCP[(MCP Servers)]
...
### 2. Azure AI Search Knowledge Base (FoundryIQ): Extractive Retrieval Only (NOT LLM)
...
│  ┌──────────────────────────────────────────────────────────┐      │
│  │   📦 Azure Blob Storage                                  │      │
│  │   • Policy markdown files uploaded from knowledge/       │      │
│  │   • Source of truth for compliance policies              │      │
│  └──────────────────────┬───────────────────────────────────┘      │
│                         │ indexed by                               │
│                         ▼                                          │
│  ┌──────────────────────────────────────────────────────────┐      │
│  │   📂 Knowledge Source                                    │      │
│  │   • Points to Azure Blob Storage container               │      │
│  │   • Automatically indexes documents on creation          │      │
...
│  │   • KB Name: AZURE_AI_KB_NAME                            │      │
│  │   • SDK: azure-search-documents==11.7.0b2                │      │
│  │   • Client: KnowledgeBaseRetrievalClient                 │      │
│  │   • Auth: DefaultAzureCredential (no API keys)           │      │
│  └──────────────────────────────────────────────────────────┘      │
│                                                                    │
│  ┌──────────────────────────────────────────────────────────┐      │
│  │   ❌ NOT USED: LLM Models (gpt-4o, etc.) for agent code  │      │
...
sequenceDiagram
    ...
    participant RAG as Azure AI Search<br/>Knowledge Base
    ...
    RAG-->>Agent: 4 policy documents (extractive results)
...
flowchart TB
    ...
    subgraph SharedServices["🌐 Shared Services"]
        subgraph FoundryIQ_Prod["Azure AI Search (FoundryIQ)"]
            BLOB_P[(Blob Storage)]
            KS_P[Knowledge Source<br/>+ local index]
            AOAI[(Knowledge Base)]
            BLOB_P --> KS_P --> AOAI
        end
...
| **Knowledge Base** | Azure AI Search KB (FoundryIQ) — extractive retrieval, NOT LLM | Same |
